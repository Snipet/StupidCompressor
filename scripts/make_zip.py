...
def  main():
  ...
  installer = "\installer\StupidCompressor Installer.exe"
   
  if demo:
    installer = "\installer\StupidCompressor Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\StupidCompressor manual.pdf" 
  ]
  ...
  ZIPNAME = "StupidCompressor-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "StupidCompressor-v" + FULLVERSIONSTR + "-win-demo.zip"
